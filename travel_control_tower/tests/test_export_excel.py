...
class ExcelExportTests(unittest.TestCase):
    def test_export_plan_to_excel_creates_expected_sheets(self) -> None:
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(workbook["携程飞猪对比"]["F4"].value, "携程显示价")
            self.assertEqual(workbook["携程飞猪对比"]["G4"].value, "飞猪显示价")
            self.assertEqual(workbook["携程飞猪对比"]["H4"].value, "携程入口")
            self.assertEqual(workbook["携程飞猪对比"]["I4"].value, "飞猪入口")
